"""
sweep_curve.py — dense open-loop single-knob sweeps + anchor frames.

Diagnostic only. No calibration, no binary search.
...
New in this version: at six anchor amplitudes (t = 0.0, 0.10, 0.20, 0.30,
0.40, 0.50), the decoded face is saved as an sRGB PNG. After every face is
done, a per-face grid figure is composited showing clean + each anchor for
every requested run. That gives you visual evidence to sit alongside the
Δa* numbers.

Usage:
    python sweep_curve.py                       # 20-face default set, A_vb + E_recipe
    python sweep_curve.py --faces 013766        # one face
    python sweep_curve.py --grid --faces 000190 # + Vb x thickness heatmap
"""
...
# Anchor amplitudes at which the flush frame is saved as an sRGB PNG.
ANCHOR_TS = (0.0, 0.10, 0.20, 0.30, 0.40, 0.50)
ANCHOR_TOL = 1e-6
...
def is_anchor(t):
    return any(abs(float(t) - a) < ANCHOR_TOL for a in ANCHOR_TS)
...
# ----------------------------------------------------------------------------
# Frame saving (linear BGR -> sRGB PNG)
# ----------------------------------------------------------------------------

def linear_bgr_to_srgb_uint8(rgb_bgr, height, width):
    """(N,3) linear-reflectance BGR tensor -> (H,W,3) uint8 sRGB RGB image."""
    img = rgb_bgr.detach().float().cpu().numpy().reshape(height, width, 3)
...
def frame_path(out_dir, face_id, run, t):
    return os.path.join(out_dir, "frames",
                        f"{face_id}_{run}_t{t:.2f}.png")


def save_frame(rgb_bgr, height, width, out_dir, face_id, run, t):
    os.makedirs(os.path.join(out_dir, "frames"), exist_ok=True)
    img = linear_bgr_to_srgb_uint8(rgb_bgr, height, width)
    Image.fromarray(img).save(frame_path(out_dir, face_id, run, t))
...
def load_face(face_id, config, bio_skin, device):
    ...
    return skin_props, reference_rgb, mask_flat, inside, (height, width)
...
def sweep_face(face_id, run_names, config, bio_skin, device, face_data, out_dir):
    C = config["chromophore_parameter"]
    skin_props, reference_rgb, mask_flat, inside, (height, width) = face_data
    ...
    # Save the clean albedo once as its own frame (used as the left column
    # of every grid).
    save_frame(reference_rgb, height, width, out_dir, face_id, "clean", 0.0)
    ...
    for run in run_names:
        ...
        for t in ts:
            ...
            # Save an sRGB PNG at the six anchor amps (per run).
            if is_anchor(t):
                save_frame(flush_rgb, height, width, out_dir,
                           face_id, run, float(t))
...
def grid_face(face_id, config, bio_skin, base, clean, face_data):
    """Run G: Vb x epidermal thickness. Kept from earlier version; no frame
    saving here -- purely diagnostic for the heatmap."""
    C = config["chromophore_parameter"]
    skin_props, _, mask_flat, inside, _ = face_data
    rows = []
...
def plot_progression(face_id, run_names, rows, out_dir):
    """Composite the anchor frames into a per-face grid.

    Rows: one per run (A_vb, E_recipe, ...).
    Columns: clean + each anchor amp.
    Each panel is annotated with its Δa* from the CSV rows.
    """
    if not run_names:
        return

    # Δa* lookup: (run, t_rounded) -> delta_a
    da = {}
    for r in rows:
        if r["run"] in run_names and r["is_clean"] == 0:
            da[(r["run"], round(float(r["t"]), 2))] = r["delta_a"]

    n_cols = 1 + len(ANCHOR_TS)                     # clean + anchors
    n_rows = len(run_names)
    fig, axes = plt.subplots(n_rows, n_cols,
                             figsize=(2.2 * n_cols, 2.5 * n_rows),
                             squeeze=False)

    clean_img_path = frame_path(out_dir, face_id, "clean", 0.0)
    clean_img = plt.imread(clean_img_path) if os.path.exists(clean_img_path) else None

    for row_i, run in enumerate(run_names):
        # Column 0: clean, once per row.
        ax = axes[row_i, 0]
        if clean_img is not None:
            ax.imshow(clean_img)
        ax.set_title("clean" if row_i == 0 else "", fontsize=9)
        ax.set_ylabel(run, fontsize=10)
        ax.set_xticks([]); ax.set_yticks([])

        for col_i, t in enumerate(ANCHOR_TS, start=1):
            ax = axes[row_i, col_i]
            p = frame_path(out_dir, face_id, run, t)
            if os.path.exists(p):
                ax.imshow(plt.imread(p))
                key = (run, round(float(t), 2))
                if key in da:
                    ax.set_xlabel(f"Δa*={da[key]:+.2f}", fontsize=8)
            if row_i == 0:
                ax.set_title(f"t={t:.2f}", fontsize=9)
            ax.set_xticks([]); ax.set_yticks([])

    fig.suptitle(f"Face {face_id} — progression at anchor amplitudes", fontsize=11)
    fig.tight_layout()
    path = os.path.join(out_dir, f"progression_{face_id}.png")
    fig.savefig(path, dpi=150)
    plt.close(fig)
    print(f"[{face_id}] progression -> {path}")
...
def main():
    ...
    ap.add_argument("--faces", nargs="+", default=[
        "003952", "032315", "039428", "051965", "043870",
        "003401", "003359", "029330", "017658", "031679",
        "033253", "013766", "022889", "021869", "033848",
        "035859", "025049", "010910", "021563", "029299",
    ])
    ap.add_argument("--runs", nargs="+", default=["A_vb", "E_recipe"],
    ...
    for face_id in args.faces:
        ...
        rows, base, clean = sweep_face(
            face_id, args.runs, config, bio_skin, device, face_data, out_dir)
        ...
        plot_progression(face_id, args.runs, rows, out_dir)
